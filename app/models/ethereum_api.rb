class EthereumApi
  ...
  def initialize
    ...
    token = Token.first
    self.contract = Ethereum::Contract.create(name: "AppToken", truffle: { paths: [ TRUFFLE_PATH ] }, client: @client, address: token.token_address)
  end

  def create_default_token!
    self.client = Ethereum::HttpClient.new(GANACHE_URL)
    self.contract = Ethereum::Contract.create(name: "AppToken", truffle: { paths: [ TRUFFLE_PATH ] }, client: @client)
    token = Token.new(
